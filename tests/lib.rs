    ...
    {$( $alg:ident, $data:expr, $expect:expr; )*} => {
        $(
            assert_eq!(
                ...
                "{} encodes correctly", $alg::to_string(),
            );
        )*
    }
}
...
fn multihash_encode() {
    assert_encode! {
        ...
        Blake2s, b"foo", "e0e4022008d6cad88075de8f192db097573d0e829411cd91eb6ec65e8fc16c017edfdb74";
    ...
    {$( $alg:ident, $hash:expr; )*} => {
        $(
            ...
            assert_eq!(
                ...
                $alg::code(),
                "{} decodes correctly", $alg::to_string()
            );
        )*
    }
}
...
fn assert_roundtrip() {
    assert_roundtrip!(
        Sha1, Sha2_256, Sha2_512, Sha3_224, Sha3_256, Sha3_384, Sha3_512, Keccak224, Keccak256,
        Keccak384, Keccak512
    );
}
